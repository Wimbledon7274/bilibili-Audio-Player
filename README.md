...
单视频、合集分P列表、甚至直播间均可解析播放（部分直播间无法正常显示直播标题。少部分直播间解析传输播放均正常，但无声音，数量少且查不到原因（v1.1版本应该解决这个问题），摆了）。
自动过滤 URL 追踪参数
...
Windows 用户，无需配置 Python 或任何代码环境，只需前往项目仓库的 Releases 页面下载打包好的exe或rar文件解压后，双击直接运行。
...
2.将任意 B 站视频、合集或直播间链接粘贴至地址栏，点击“开始解析”。工具会自动剔除 spm_id_from 等长尾跟踪参数。自动保存近10次解析连接存放在历史记录。
3. 
   - 列表双击播放，支持关键字过滤搜索。
   - 支持单曲循环、列表循环、顺序播放。
...
4.多次解析412错误。可以尝试更新SESSDATA。或等待发布更新YT-DLP新版本。
  ...
  YT-DLP重大更新时才会进行后续更新(V1.21及之后版本优先调用官方API，yt-dlp仅作为保底解析使用）。不支持其他更新方式，仓库下载后替换文件即可。
